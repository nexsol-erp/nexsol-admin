...
const BranchAssignmentPage = () => {
  ...
  const [assigned, setAssigned] = useState([]); // array of branch codes
  ...
  useEffect(() => {
    ...
    const fetchBranches = async () => {
      try {
        ...
        // Normalize: allow [ ... ] or { branches: [...] } or { data: [...] }
        const list =
          Array.isArray(data) ? data : data.branches || data.data || [];
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        // Normalize: support either [ ... ] or { users: [...] } or { data: [...] }
        const list =
  ...
  // Load selected user's branch list
  useEffect(() => {
    ...
    const fetchUserBranches = async () => {
      try {
        ...
        const response = await fetch(
          `/api/${tenancyId}/admin/users/${selectedUserId}/branches`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
        ...
        if (Array.isArray(data)) {
          ...
          // { branches: ["B1","B2"] } or { branches: [{branchCode:...}] }
        ...
        console.error(e);
        setError("Failed to load user branch assignments");
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to save branch assignments");
      }

      setOriginalAssigned(assigned);
      setSuccessMsg("Branch assignments updated.");
    } catch (e) {
      console.error(e);
